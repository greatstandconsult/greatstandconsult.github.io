import {auth,db} from "./firebase.js?v=17.1";
import {initializeApp,getApps} from "https://www.gstatic.com/firebasejs/12.1.0/firebase-app.js";
import {getAuth,signInWithEmailAndPassword,onAuthStateChanged,signOut,createUserWithEmailAndPassword} from "https://www.gstatic.com/firebasejs/12.1.0/firebase-auth.js";
import {doc,getDoc,collection,getDocs,addDoc,updateDoc,deleteDoc,setDoc,serverTimestamp,query,orderBy,where} from "https://www.gstatic.com/firebasejs/12.1.0/firebase-firestore.js";
...
function isAdminRole(){
  const role=normalizeRole(window.currentUserRole || "");
  return role === "admin" || role === "superadmin";
}

function isSuperAdminRole(){
  return normalizeRole(window.currentUserRole || "") === "superadmin";
}
...
function parsePastedQuestions(raw){
  ...
  for(const line of lines){
    ...
    const em=line.match(/^(?:EXPLANATION|CORRECTION|EXPLANATION\s*\/\s*CORRECTION|WHY)\s*[:=\-]?\s*(.*)$/i);
...
function setCbtMode(mode){
  ...
  // Prevent hidden manual fields from blocking form submission through HTML5 required validation.
