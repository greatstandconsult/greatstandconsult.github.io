...
// V18.2: optional CBT explanation parsing.
// This is deliberately isolated from authentication and Firebase initialization.
function getCBTExplanationFromLine(line) {
  const match = String(line || "").match(
    /^\s*(?:EXPLANATION|Correct\s*Explanation)\s*[:=]\s*(.+)\s*$/i
  );
  return match ? match[1].trim() : "";
}

function getCBTAnswerFromLine(line) {
  const match = String(line || "").match(
    /^\s*(?:ANSWER|Correct\s*Answer)\s*[:=]\s*([A-D])\s*$/i
  );
...
function buildCBTExplanationHTML(questions) {
  if (!Array.isArray(questions)) return "";
  return questions.map((q, i) => {
    const explanation = q && (q.explanation || q.Explanation) || "";
    if (!explanation) return "";
    return `<div class="cbt-explanation">
      <strong>Explanation:</strong> ${String(explanation).replace(/[&<>"']/g, c => ({
        "&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"
      }[c]))}
    </div>`;
  }).join("");
