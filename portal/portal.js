...
async function loadStudentResultsIfNeeded(){if((window.currentUserRole||"student")!=="student")return;const list=$("studentResultsList");list.innerHTML='<p class="muted">Loading results...</p>';try{const s=await getDocs(collection(db,"results"));list.innerHTML="";let found=0;s.forEach(d=>{const r=d.data();if(r.studentId!==auth.currentUser.uid)return;found++;const c=document.createElement("article");c.className="result-card";const h=document.createElement("h3");h.textContent=r.assignmentTitle||"Assignment Result";const sc=document.createElement("div");sc.className="score";sc.textContent=String(r.score??0)+"/100";const fb=document.createElement("div");fb.className="feedback";fb.textContent=r.feedback||"No feedback provided.";c.append(h,sc,fb);list.appendChild(c)});if(!found)list.innerHTML='<p class="muted">No results available yet.</p>'}catch(e){console.error(e);list.innerHTML='<p class="message">Could not load results: '+(e.code||e.message)+'</p>'}}
