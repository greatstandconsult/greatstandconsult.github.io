...
async function submitTest(autoSubmit){if(!currentTest||!auth.currentUser)return;clearInterval(testTimerInterval);$('submitTestBtn').disabled=true;$('testSubmitMessage').textContent=autoSubmit?'Submitting...':'Submitting test...';try{const qs=currentTest.questions||[];let correct=0;qs.forEach((q,i)=>{if(testAnswers[i]===q.correct)correct++});const score=qs.length?Math.round(correct/qs.length*100):0;await addDoc(collection(db,'results'),{resultType:'cbt',testId:currentTest.id,testTitle:currentTest.title||'CBT Test',subject:currentTest.subject||'General',studentId:auth.currentUser.uid,studentEmail:auth.currentUser.email||'',score,correctAnswers:correct,totalQuestions:qs.length,answers:testAnswers,submittedAt:serverTimestamp(),updatedAt:serverTimestamp(),createdAt:serverTimestamp()});$('testSubmitMessage').className='message submission-success';$('testSubmitMessage').textContent=`Test submitted successfully ✅ Score: ${score}/100 (${correct}/${qs.length})`;$('submitTestBtn').textContent='Submitted';await loadStudentResultsIfNeeded();await loadCbtResultsIfNeeded();}catch(e){console.error(e);$('testSubmitMessage').className='message submission-error';$('testSubmitMessage').textContent='Could not submit test: '+(e.code||e.message)}finally{$('submitTestBtn').disabled=false}}
async function loadCbtResultsIfNeeded(){const list=$('testResultsList');if(!list)return;if((window.currentUserRole||'student').toLowerCase()!=='admin'&&(window.currentUserRole||'student').toLowerCase()!=='superadmin')return;list.innerHTML='<p class="muted">Loading CBT results...</p>';try{const s=await getDocs(collection(db,'results'));const rows=s.docs.map(d=>({id:d.id,...d.data()})).filter(r=>r.resultType==='cbt');if(!rows.length){list.innerHTML='<p class="muted">No CBT results yet.</p>';return}list.innerHTML='';rows.forEach(r=>{const c=document.createElement('article');c.className='result-card';const h=document.createElement('h3');h.textContent=r.testTitle||'CBT Test';const meta=document.createElement('p');meta.className='submission-meta';meta.textContent='Student: '+(r.studentEmail||r.studentId||'Unknown');const sc=document.createElement('div');sc.className='score';sc.textContent=String(r.score??0)+'/100';const detail=document.createElement('div');detail.className='feedback';detail.textContent=(r.correctAnswers??0)+' / '+(r.totalQuestions??0)+' correct';c.append(h,meta,sc,detail);list.appendChild(c)})}catch(e){console.error(e);list.innerHTML='<p class="message">Could not load CBT results: '+(e.code||e.message)+'</p>'}}

onAuthStateChanged(auth,async user=>{if(!user){loginView.classList.remove("hidden");dashboardView.classList.add("hidden");logoutBtn.classList.add("hidden");return}loginView.classList.add("hidden");dashboardView.classList.remove("hidden");logoutBtn.classList.remove("hidden");try{const s=await getDoc(doc(db,"users",user.uid));if(!s.exists()){adminPanel.style.display="none";$("welcomeTitle").textContent="PROFILE NOT FOUND";return}const p=s.data(),role=String(p.role||"student").trim().toLowerCase(),allowed=role==="admin"||role==="superadmin";window.currentUserRole=role;window.currentUserRole=role;$("welcomeTitle").textContent="Welcome, "+(p.name||user.email);$("roleText").textContent="Signed in as "+role;adminPanel.style.display=allowed?"block":"none";assignmentAdminPanel.style.display=allowed?"block":"none";testAdminPanel.style.display=allowed?"block":"none";$("submissionsAdminPanel").style.display=allowed?"block":"none";$("testResultsAdminPanel").style.display=allowed?"block":"none";$("statusText").textContent=role==="superadmin"?"SUPERADMIN ACCOUNT DETECTED ✅":role==="admin"?"ADMIN ACCOUNT DETECTED ✅":"Student account detected.";$("notesCount").textContent=await getCount("notes");$("assignmentsCount").textContent=await getCount("assignments");$("testsCount").textContent=await getCount("tests");$("resultsCount").textContent=await getCount("results");await loadNotes();await loadAssignments();await loadTests();if(allowed){await loadSubmissions();await loadCbtResultsIfNeeded()}await loadStudentResultsIfNeeded()}catch(e){console.error(e);adminPanel.style.display="none";assignmentAdminPanel.style.display="none";testAdminPanel.style.display="none";$("submissionsAdminPanel").style.display="none";$("testResultsAdminPanel").style.display="none";$("statusText").textContent="Firestore error: "+(e.code||e.message)}});
