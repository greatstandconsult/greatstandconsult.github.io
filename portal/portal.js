...
function parseCBTExplanationLine(line) {
  const m = String(line || "").match(
    /^\s*(?:EXPLANATION|Explanation|Correct\s*Explanation)\s*[:=]\s*(.+)$/i
  );
  return m ? m[1].trim() : null;
}

function parseCBTAnswerLine(line) {
  const m = String(line || "").match(
    /^\s*(?:ANSWER|Answer|Correct\s*Answer)\s*[:=]\s*([A-D])\s*$/i
  );
...
function renderCBTPostTestCorrections(questions, selectedAnswers) {
  ...
  return questions.map((q, i) => {
    const selected = String(
      selectedAnswers?.[i] ?? selectedAnswers?.[String(i)] ?? ""
    ).toUpperCase();
    const correct = String(q.answer ?? q.correctAnswer ?? "").toUpperCase();
    const explanation = q.explanation ?? "";
    const status = selected && correct && selected === correct ? "✓ Correct" : "✗ Incorrect";
    const answerText = correct ? `Correct answer: ${esc(correct)}` : "";
    return `<div class="cbt-correction">
      <div><strong>Question ${i + 1}: ${status}</strong></div>
      ${answerText ? `<div>${answerText}</div>` : ""}
      ${explanation ? `<div class="cbt-explanation"><strong>Explanation:</strong> ${esc(explanation)}</div>` : ""}
    </div>`;
  }).join("");
}
