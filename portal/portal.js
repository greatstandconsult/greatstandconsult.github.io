  ...
  try{
    ...
    s.forEach(d=>{
      ...
      if(isAdminRole()){
        ...
        del.addEventListener("click",async()=>{if(!confirm("Delete this assignment? This cannot be undone."))return;del.disabled=true;try{await deleteDoc(doc(db,"assignments",d.id));await loadAssignments();$("assignmentsCount").textContent=await getCount("assignments")}catch(e){console.error(e);alert("Could not delete assignment: "+(e.code||e.message));del.disabled=false}});
