...
function startTimer(minutes){clearInterval(testTimerInterval);testSecondsLeft=Math.max(1,Number(minutes||30)*60);updateTimer();testTimerInterval=setInterval(()=>{testSecondsLeft--;updateTimer();if(testSecondsLeft<=0){clearInterval(testTimerInterval);testTimerInterval=null;$('testSubmitMessage').textContent='Time is up. Submitting your test...';void submitTest(true)}},1000)}
